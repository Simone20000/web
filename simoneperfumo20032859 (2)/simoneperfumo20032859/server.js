...
// Get all progetti
// Request body: empty
// Response body: array of objects representing all the progs
...
app.get('/api/acquisti',(req,res)=>{ 
  db.getAllAcquisti().then((acquisti)=>{
    if (acquisti.error){
      res.status(404).json(acquisti);
    } else {
      res.json(acquisti);
  }}).catch( (err) => {
...
app.get('/api/progetti/segui',(req,res)=>{ 
  db.getAllSeguiti().then((seguiti)=>{
    if (seguiti.error){
      res.status(404).json(seguiti);
    } else {
      res.json(seguiti);
  }}).catch( (err) => {
...
app.get('/api/documenti',(req,res)=>{ 
  db.getAllDocumenti().then((documenti)=>{
    if (documenti.error){
...
// Get all documenti
// Request body: empty
// Response body: array of objects representing all the docs
app.get('/api/documenti/:ID',(req,res)=>{ 
  db.getDocumentiID(req.params.ID).then((documenti)=>{
    if (documenti.error){
      res.status(404).json(documenti);
    } else {
      res.json(documenti);
...
// Request body: empty
// Response body: array of objects representing all the docs
app.get('/api/donazioni/:ID',(req,res)=>{ 
  db.getDonazioniID(req.params.ID).then((donazioni)=>{
    if (donazioni.error){
      res.status(404).json(donazioni);
    } else {
      res.json(donazioni);
  }}).catch( (err) => {
  ...
  if (!errors.isEmpty()) {
    return res.status(422).json({errors: errors.array()});
      ...
      const documento = {
        ...
        ID_creatore:req.body.ID_creatore
      };
...
app.post('/api/acquisti',isLoggedIn, [
      ],(req, res) => {
      console.log(req.body);
      const errors = validationResult(req); // format error message
      if (!errors.isEmpty()) {
        return res.status(422).json({errors: errors.array()});
          ...
          acquisto.ID_FIN=req.user.ID
          // progetto.ID_creatore=req.user.ID;
          db.createAcquisto(acquisto).then((ID) => {
            // 201 -> Created 
            // Location Header redirects to the newly reported response
            res.status(201).json({'ID': ID});
  ...
  app.post('/api/progetti/segui',isLoggedIn, [
  ],(req, res) => {
  console.log(req.body);
  const errors = validationResult(req); // format error message
  if (!errors.isEmpty()) {
    return res.status(422).json({errors: errors.array()});
      ...
      seguito.ID_FIN=req.user.ID
      // progetto.ID_creatore=req.user.ID;
      db.createSeguito(seguito).then((ID) => {
        // 201 -> Created 
        // Location Header redirects to the newly reported response
        res.status(201).json({'ID': ID});
...
app.post('/api/progetti',isLoggedIn, [
  check('descrizione').isString().isLength({ max: 32 }),
  check('titolo').isString().isLength({ max: 12 }),
],
   (req, res) => {
  console.log(req.body);
  const errors = validationResult(req); // format error message
  if (!errors.isEmpty()) {
    return res.status(422).json({errors: errors.array()});
      ...
      progetto.ID_creatore=req.user.ID
      // progetto.ID_creatore=req.user.ID;
      if(req.user.creatore==0) return res.status(422).json({errors: errors.array()});
  ...
  app.delete('/api/progetti/:ID',isLoggedIn,(req,res)=> {
    db.deletePROGETTO(req.params.ID,req.user.ID).then( (result) => {
      db.deleteDOCProgetti(req.params.ID)
            // 204 -> No content
            res.status(204).end();
        }).catch( (err) => {
  ...
  app.delete('/api/progetti/segui/:ID',isLoggedIn,(req,res)=> {
    db.removeFollow(req.params.ID).then( (result) => {
            // 204 -> No content
            res.status(204).end();
        }).catch( (err) => {
  ...
  app.post('/api/login', function(req, res, next) {
      passport.authenticate('local', function(err, user, info) {
          ...
          req.login(user, function(err) 
          {
              ...
              return res.json({ID : req.user.ID, nome : req.user.nome, cognome : req.user.cognome, email: req.user.email, creatore : req.user.creatore});
          });
      })(req, res, next);
  });
...
(req, res) => {   const errors = validationResult(req);  
  ...
  udb.getUserById(userID).then((user) => {
      ...
      {
          res.json({nome : user.nome, cognome: user.cognome, email : user.email ,creatore: user.creatore});
      }
  }).catch( (err) => {
      res.status(500).json({ 
          'errors': [{'param': 'Server', 'msg': err}],
...
app.post('/api/register', [
  check('nome').isString().isLength({ max: 12 }),
  check('cognome').isString().isLength({ max: 12 }),
  check('email').isEmail(),
